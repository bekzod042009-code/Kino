...
async def send_content_by_code(message: Message, code: str) -> bool:
    ...
    if series:
        ...
        if not episodes:
            return False
        await message.answer(f"🎬 {title}\n\n📥 Barcha qismlar yuborilmoqda...")
        for ep_num, file_id in episodes:
            await message.answer_video(video=file_id, caption=f"{title} — {ep_num}-qism")
            await asyncio.sleep(0.3)  # flood limitga tushmaslik uchun kichik pauza
        return True
