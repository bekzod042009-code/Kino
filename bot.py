...
async def _tmdb_try(session: aiohttp.ClientSession, media_type: str, title: str):
    try:
        ...
        if countries:
            country = ", ".join(c["name"] for c in countries)
        elif detail.get("origin_country"):
            country = ", ".join(detail["origin_country"])
...
def format_info_preview(info) -> str:
    ...
    if info.get("language"):
        lines.append(f"🗣 Til: {info['language']}")
    if info.get("rating"):
...
def build_info_caption(row, icon="🎬", extra_line=""):
    row = dict(row)
    ...
    if row.get("trailer_url"):
        lines.append(f"\n🎞 Treyler: {row['trailer_url']}")
    return "\n".join(lines)


async def post_to_channel(code, row, is_series=False, total_episodes=0):
    ...
    extra_line = f"🔢 Jami qismlar: {total_episodes}" if is_series else ""
...
async def add_movie_start(message: Message, state: FSMContext):
    ...
    await message.answer(
        ...
        "poster, tavsif, reyting, janr va boshqa ma'lumotlarni qidiradi."
    )
...
async def finish_series_episodes(message: Message, state: FSMContext):
    ...
    if is_new and series:
        await post_to_channel(code, series, is_series=True, total_episodes=total)
...
# ---- KANAL TAHRIRLASH (username'ini almashtirish) ----
