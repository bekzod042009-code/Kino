...
async def start_handler(message: Message, command: CommandObject, state: FSMContext):
    ...
    if await is_subscribed(message.from_user.id):
        if deep_link_code:
            await send_movie_by_code(message, deep_link_code)
        else:
...
async def send_movie_by_code(message: Message, code: str) -> bool:
    """Kod bo'yicha kinoni topib yuboradi. Topilsa True, topilmasa False qaytaradi."""
...
async def check_sub_handler(call: CallbackQuery, state: FSMContext):
    if await is_subscribed(call.from_user.id):
        ...
        if pending_code:
            ...
            found = await send_movie_by_code(call.message, pending_code)
            if not found:
...
def search_results_kb(movies) -> InlineKeyboardMarkup:
    # Har bir kino uchun raqamli tugma yaratadi (1, 2, 3, ...)
    # callback_data ichida kino kodi to'g'ridan-to'g'ri saqlanadi
    buttons = []
    row = []
    for i, (code, title) in enumerate(movies, start=1):
        row.append(InlineKeyboardButton(text=str(i), callback_data=f"pick:{code}"))
...
async def pick_movie_handler(call: CallbackQuery):
    ...
    if movie:
        ...
        await call.answer("❌ Kino topilmadi, o'chirilgan bo'lishi mumkin.", show_alert=True)


# ================= KINO KODI / NOMI QABUL QILISH =================
...
async def code_handler(message: Message):
    ...
    # 1) Avval aniq kod bo'yicha qidiramiz (masalan "583")
    ...
    if results:
        ...
        for i, (code, title) in enumerate(results, start=1):
            text_lines.append(f"{i}. {title}")
        text_lines.append("\n👇 Kerakli raqamni tanlang:")
        await message.answer("\n".join(text_lines), reply_markup=search_results_kb(results))
    else:
        await message.answer("❌ Bunday kino topilmadi. Kod yoki nomini qaytadan tekshiring.")
