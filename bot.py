...
def add_submission(user_id, name, file_id, caption):
    db_run(
        "INSERT INTO submissions (user_id, name, file_id, caption, created_at) VALUES (?, ?, ?, ?, ?)",
        (user_id, name, file_id, caption, datetime.now().isoformat()),
    )


def get_all_submissions():
    return db_run("SELECT id, user_id, name, file_id, caption FROM submissions ORDER BY id")
...
async def send_content_by_code(message: Message, code: str, user_id: int = None) -> bool:
    ...
    if movie:
        ...
        await message.answer_video(
            video=movie["file_id"], caption=caption,
            protect_content=protect,
...
async def add_movie_title(message: Message, state: FSMContext):
    ...
    if info:
        ...
        await message.answer("🎥 Endi videoni (kino faylini) yuboring:", reply_markup=cancel_kb())
    else:
...
async def show_submissions(message: Message):
    ...
    if not rows:
        ...
        return
    for sid, uid, name, file_id, caption in rows:
        kb = InlineKeyboardMarkup(inline_keyboard=[[
        ...
        if caption:
            cap += f"\n📝 {caption}"
        await message.answer_video(video=file_id, caption=cap, reply_markup=kb)
...
async def submit_start(message: Message, state: FSMContext):
    ...
    await message.answer(
        "Kino faylini (videoni) yuboring. Admin ko'rib chiqib, yoqsa botga rasman qo'shadi.\n"
        "Xohlasangiz video ostiga izoh (kino nomi) ham yozib qo'yishingiz mumkin."
    )


@dp.message(UserStates.submit_wait, F.video)
async def submit_save(message: Message, state: FSMContext):
    ...
    add_submission(message.from_user.id, name, message.video.file_id, caption)
