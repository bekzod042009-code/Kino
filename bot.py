...
def share_kb(code: str, title: str) -> InlineKeyboardMarkup:
...
async def post_to_channel(code, row, is_series=False, total_episodes=0, total_seasons=0):
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[[
        InlineKeyboardButton(text="🎬 Ko'rish", url=f"https://t.me/{BOT_USERNAME}?start={code}")
    ]])
...
async def eppick_handler(call: CallbackQuery):
    ...
    if file_id:
        ...
        await call.message.answer_video(
            ...
            reply_markup=share_kb(series_code, title),
        )
...
async def send_content_by_code(message: Message, code: str, user_id: int = None) -> bool:
    ...
    if movie:
        ...
        await message.answer_video(
            ...
            protect_content=protect,
            reply_markup=share_kb(code, movie["title"]),
        )
...
async def finish_series_episodes(message: Message, state: FSMContext):
    ...
    if return_state == "edit":
        ...
        if series:
            await post_to_channel(code, series, is_series=True, total_episodes=total, total_seasons=total_seasons)
